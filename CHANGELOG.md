...
## [26.5.8-alpha.1] — 2026-05-08

### Fixed
- **Saved offsets/rotations moved the part in the wrong direction after Save+reload.** User report: "I corrected some offsets, then saved, and when the 3D viewport reloaded the offsets were not saved in the right directions so the part moved in the wrong direction." Empirical investigation (running synthetic 1mm offsets through `kicad-cli pcb export glb` inside the visual-verify Docker image) showed kicad-cli's bake mapping is `+X mm → world +X`, **`+Y mm → world −Z`** (sign flip), `+Z mm → world +Y`, and rotations: `+X rotate → world rotate about −X`, `+Y rotate → world rotate about +Z`, **`+Z rotate → world rotate about −Y`**. The live preview's `applyLiveDelta` (in `src/blocks/Model3DViewerGL.tsx`) was using `dzWorld = +dyKicad` (and matching wrong signs on X+Z rotations), so the live three.js position diverged from what kicad-cli would bake on Save → ~2 mm drift (or visibly-wrong rotation) the moment the file round-tripped. Fix: corrected `applyLiveDelta` and `kicadAxisToWorld` (hover-arrow helper) to match the empirical kicad-cli mapping, and flipped the matching wedge `sign` fields in `Model3DJogDial.tsx` (Y wedges + ArrowUp/Down keys) and `Model3DRotateDial.tsx` (X-rotate, Z-rotate wedges) so user-visible click semantics still produce screen-relative motion under the corrected mapping.
- **Locked-in regression: live-vs-baked screenshot equality.** Extended the visual-verify harness with a `saveAfterAction: true` fixture flag — after the standard live-jog snapshot, the runner clicks `[data-testid="positioner-save"]`, waits for `__model3dGLLoadCount` to strictly increment (proves the GLB reload actually happened), captures a third "BAKED" snapshot, and asserts each chip's world position drift between LIVE and BAKED ≤ `liveVsBakedMaxDelta` (default 50 µm), pairing chips by name (uuids change across reload). Two fixtures cover the two surviving sign flips: `synthetic_save_reload_equality_y` (jog +Y) and `synthetic_save_reload_equality_rotate_z` (rotate +Z). Verified: pre-fix builds **fail** with chip drift Δz=±2.000e−3 m on Y; post-fix builds **pass** with biggest-axis-delta = 0 m. Any future regression in the live↔bake mapping fails CI loudly.
...
### Fixed
- **IPEX (and any pre-alpha.5) footprint rendered "on its side" in the 3D viewer.** Wave 8-A's `compute_step_pad_offset` only fired at *drop* time — committed footprints whose `(model …)` block lacks an `(offset (xyz …))` sub-expr (the legacy SnapEDA shape) still rendered at the STEP origin, which for SnapEDA STEPs is body-centroid not pin-1. Fix: `render_3d._rewrite_or_strip_model_blocks_with_warnings` now scans each kept `(model …)` block for a missing `(offset …)` sub-expr and computes the pad-centre→body-centre offset in-memory at render time (disk untouched). A structured `{kind: "auto_offset_applied", model_path, offset:[x,y,z]}` warning surfaces via `bundle["warnings"]`. Required a sibling fix in `drop_import._footprint_pad_bbox`: kiutils refuses to parse footprints with offset-less `(model …)` blocks (`Expression does not have the correct type`), so a regex `_PAD_AT_RE` fallback was added.
- **IPEX 3D model load failure was completely silent.** `Model3DViewerGL.loadGLB` consumed the sidecar's `glb_data_url` and `top_layers_svg_data_url` but discarded `r.warnings`. So when an assembly STEP triggered `tessellation_failed` (RWGltf_CafWriter skips nodes lacking pre-computed triangulation — common for cadquery-style STEPs and the user's IPEX) or path resolution failed (`model_not_found`), the user saw a green PCB with no chip and zero diagnostic. KiCad's native View 3D worked because it uses OCCT's real-time renderer, not RWGltf_CafWriter — exactly the asymmetry the user reported. Fix: a new amber non-blocking banner pinned top-left over the canvas surfaces every warning kind ("3D model partially missing — N warning(s) from kicad-cli", per-warning bullets with sibling-match hints when applicable), with × dismiss. Helper extracted to `_renderWarnings.ts` for testability. Window probe `__model3dGLLastWarnings` exposed for harness assertions.

### Added
- **Z-reset disk button** between `+Z 0.1mm` and `−Z 0.1mm` in the Model3DJogZ column. The existing centre-reset disk in the XY dial only zeroed translateX + translateY (per its docstring), forcing the user to wheel back through dozens of 0.1mm ticks if a chip ended up far off in Z. The new "0" disk zeroes Z while preserving X+Y. `data-testid="jog-z-reset"`.
- **Model3DRotateDial.tsx** — pure-SVG ±90° rotation dial sitting beside Model3DJogZ. Six wedges (±X / ±Y / ±Z) at 60° angular spacing, opposing-sign pairs 180° apart, purple palette to distinguish from the XY-translate dial's blue. Centre RESET disk zeroes all three rotation axes. Rotation values wrap to `[-180, 180)` for numerical stability. Wired through Model3DPositioner via two new pulse-prop pairs (`rotateJogDelta`/`onRotateJogConsumed` and `forceRotation`/`onForceRotationConsumed`). 8 new vitest specs cover the reducer logic; Playwright `regressions.spec.ts` adds an end-to-end click-and-verify spec.
- **PCFSoftShadowMap shadow casting** on the chip body (cast) and substrate (receive). Key directional light at `(3, 10, 3)` is now `castShadow=true` with `mapSize 2048²`, ortho frustum ±0.05 m, near 0.1 / far 20 m, `bias=-1e-5`, `normalBias=1e-4`, `radius=4` — sized for kicad-cli's metres-scale GLBs. Adds a `HemisphereLight(0xa6c9b3, 0x1a1a1a, 0.20)` for cool/warm separation. Note: software-renderer (llvmpipe in the visual-verify container) under-samples PCF; hardware GPUs will show stronger contact shadows than the harness PNGs.

### Changed
- **PCB substrate + chip body now render at 90% opacity** (`opacity:0.9, transparent:true, depthWrite:true`). Replaces the alpha.5 substrate-opaque-and-everything-else-default sweep, which had a legacy "force opaque if opacity ≥ 0.7" line that fought any per-mesh transparency. Substrate keeps the deep KiCad-green clamp + roughness 0.55 / metalness 0; chip body inherits the upstream PBR but at 0.9 alpha so internal geometry (pin contacts, body cavities) is faintly visible — matches user request "I would like the opacity of the PCB and footprint to be 90%".
- **Lighting toned down** to address "the image is still a bit bright". `toneMappingExposure 0.95 → 0.75`, `AmbientLight 0.25 → 0.15`, key-directional `0.55 → 0.75` (slightly brighter to keep contact shadows visible after the exposure cut).

### Notes
- 4 lanes ran in parallel via background agents (Wave 9-A IPEX warnings, 9-B footprint placement, 9-C UI controls, 9-D opacity+shading). All edits to `Model3DViewerGL.tsx` were lane-coordinated by code region: 9-A owned `loadGLB` + warning overlay; 9-D owned per-mesh material loop + lighting init; no merge conflicts.
- Final test count: **297 passed, 2 skipped** (sidecar pytest, +2 from auto-offset coverage); **25 vitest specs** (5 fallback + 12 warnings + 8 rotate-dial reducer); **1 new Playwright wave9c-positioner regression** (DOM presence + rotation + reset).
